...
function displayNewsArticles($articles) {
    foreach ($articles as $article) {
        echo '<div class="latest-news_article-wrapper">
...
function displayFormMessages($errors) {

    $className = count($errors) == 0 ? "form-success" : "form-error";
    if (count($errors) == 0) $errors[] = "Your message has been sent!";

    foreach ($errors as $message) {
        echo '<div class="contact_form-message ' . $className . '">'
        . '<button type="button" class="form-button" onclick="closeMessage(this)">×</button>'
        . $message
        . '</div>';
    }
}
